...
def load_training_log():
    path = os.path.join(BASE, "results", "dqn_training_log.csv")
...
def load_eval_results():
    path = os.path.join(BASE, "results", "evaluation_results.csv")
    if os.path.exists(path):
        return pd.read_csv(path)
    return None
...
if train_df is not None:
    ...
    st.warning("Training log not found!")
...
fixed_gif_path = os.path.join(BASE, "results", "gifs", "fixed_timer.gif")
dqn_gif_path   = os.path.join(BASE, "results", "gifs", "dqn_agent.gif")
...
with col1:
    ...
    if os.path.exists(fixed_gif_path):
        with open(fixed_gif_path, "rb") as f:
            st.image(f.read(), caption="Fixed Timer — switches every 30s regardless of traffic", use_container_width=True)
    else:
        st.warning("Fixed Timer GIF not found!")

with col2:
    st.markdown("### 🟢 DQN Agent (Trained)")
    if os.path.exists(dqn_gif_path):
        with open(dqn_gif_path, "rb") as f:
            st.image(f.read(), caption="DQN Agent — dynamically adapts to traffic conditions", use_container_width=True)
    else:
        st.warning("DQN GIF not found!")
